...
`define PWR .iovdd(IOVDD),.iovss(IOVSS),.vdd(VDD),.vss(VSS),
  (* keep *) sg13g2_IOPadIn clk_pad(`ifdef USE_POWER_PINS `PWR `endif .p2c(clk),.pad(clk_PAD));
  (* keep *) sg13g2_IOPadIn rst_n_pad(`ifdef USE_POWER_PINS `PWR `endif .p2c(rst_n),.pad(rst_n_PAD));
  genvar k;
  generate for(k=0;k<8;k=k+1) begin: inputs
    (* keep *) sg13g2_IOPadIn input_pad(`ifdef USE_POWER_PINS `PWR `endif .p2c(din[k]),.pad(input_PAD[k]));
  end endgenerate
  generate for(k=0;k<8;k=k+1) begin: outputs
    (* keep *) sg13g2_IOPadOut30mA output_pad(`ifdef USE_POWER_PINS `PWR `endif .c2p(dout[k]),.pad(output_PAD[k]));
  end endgenerate
