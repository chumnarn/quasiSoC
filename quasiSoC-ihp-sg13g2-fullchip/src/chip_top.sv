module chip_top(
  ...
  sg13g2_IOPadIn clk_pad(.p2c(clk), .pad(clk_PAD));
  sg13g2_IOPadIn rst_n_pad(.p2c(rst_n), .pad(rst_n_PAD));

  genvar k;
  generate for(k=0;k<8;k=k+1) begin: inputs
    sg13g2_IOPadIn input_pad(.p2c(din[k]), .pad(input_PAD[k]));
  end endgenerate

  generate for(k=0;k<8;k=k+1) begin: outputs
    sg13g2_IOPadOut30mA output_pad(.c2p(dout[k]), .pad(output_PAD[k]));
  end endgenerate
